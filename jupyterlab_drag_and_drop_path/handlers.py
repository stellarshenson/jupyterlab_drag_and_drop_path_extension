"""
API handlers for drag-and-drop path resolution.

Provides two endpoints used by the frontend extension:

- ``server-info``  - the absolute server root directory, used to turn
  file-browser contents paths into absolute filesystem paths
- ``terminal-cwd/<name>`` - the current working directory of a terminal,
  used to turn an absolute path into a path relative to that terminal
"""
import json
import os
import subprocess
...
class ServerInfoHandler(APIHandler):
    ...
    async def get(self):
        """Return the absolute server root directory."""
...
def setup_handlers(web_app):
    ...
    server_info_route = url_path_join(
        base_url, "api", "drag-and-drop-path", "server-info"
    )
    terminal_cwd_route = url_path_join(
        base_url,
        "api",
        "drag-and-drop-path",
        "terminal-cwd",
    ...
    handlers = [
        (server_info_route, ServerInfoHandler),
        (terminal_cwd_route, TerminalCwdHandler),
    ]
